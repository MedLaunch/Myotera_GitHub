...
constexpr uint8_t COMMAND_CHAR_UUID[] = {0x01, 0x00};
constexpr uint16_t commandCharUUID16 = 0x0001;
constexpr uint8_t DATA_CHAR_UUID[] = { 0x02, 0x00};
    ...
    switch(resourceId.localResourceId)
    {
        case WB_RES::LOCAL::COMM_BLE_GATTSVC_SVCHANDLE::LID:
        {
            ...
            for (size_t i=0; i<svc.chars.size(); i++)
            {
                ...
                uint16_t uuid16;
                if (c.uuid.size()==2)
                    uuid16 = *reinterpret_cast<const uint16_t*>(&(c.uuid[0]));
                else
                    uuid16 = *reinterpret_cast<const uint16_t*>(&(c.uuid[12]));
...
{
    for (int i=0;i<MAX_DATASUB_COUNT; i++)
    {
